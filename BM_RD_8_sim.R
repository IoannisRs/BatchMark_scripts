...
nij = round(nij)
T_K = c(8,8,8,8,8,8)
T_K_0 = c(0,8,8,8,8,8,8)

p_true = rep(p_true_true, each = 8)
...
for(runs in 1:100){
  ...
  V_runs[[runs]] = V_store
  W_runs[[runs]] = W_store
  saveRDS(p_runs, file = paste("p_runs_BM_sim_alt_unif_bias_8_so",".RDS"))
  saveRDS(w_runs, file = paste("abund_runs_BM_sim_alt_unif_bias_8_so",".RDS")) 
  saveRDS(V_runs, file = paste("V_runs_alt_unif_bias_8_so",".RDS")) 
  saveRDS(W_runs, file = paste("W_runs_alt_unif_bias_8_so",".RDS")) 
